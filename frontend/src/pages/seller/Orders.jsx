        ...
        <div className='no-scrollbar flex-1 h-[95vh] overflow-y-scroll'>
            <div className="md:p-10 p-4 space-y-4">
                ...
                {orders.map((order, index) => {
                    const baseAmount = order.items.reduce((acc, item) => {
                        return acc + item.quantity * (item.product.price - item.product.offerPrice);
                    }, 0);
                    ...
                    return (
                        <div key={index} className="flex flex-col md:items-center md:flex-row gap-5 justify-between p-5 max-w-4xl rounded-md border border-gray-300">
                            ...
                            <div className="text-black font-medium text-sm md:text-base space-y-1 my-auto text-center">
                                <p className="text-lg">{currency}{totalAmount.toFixed(2)}</p>
                            </div>
